...
export const handleIncompleteBold = (text: string): string => {
    ...
    const openingMarker = boldMatch[1];
    const contentAfterMarker = boldMatch[2];
    if (openingMarker === undefined || contentAfterMarker === undefined) {
        return text;
    }
    const markerIndex = text.lastIndexOf(openingMarker);
...
export const handleIncompleteDoubleUnderscoreItalic = (
    ...
    if (!italicMatch) {
        ...
        if (halfCompleteMatch) {
            const openingMarker = halfCompleteMatch[1];
    ...
    const openingMarker = italicMatch[1];
...
export const handleIncompleteBoldItalic = (text: string): string => {
    ...
    const openingMarker = boldItalicMatch[1];
